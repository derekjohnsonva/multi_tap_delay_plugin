...
- **Done:**
  ...
  - **Validation:** both validators clean. `clap-validator validate` — **16 passed, 0 failed, 5 skipped** (skips are note-port checks, N/A for an audio FX). `pluginval` v1.0.4 (installed to `~/.local/bin`) at **strictness level 10 — SUCCESS** (incl. parameter fuzzing, multiple sample-rate/block-size combos, bus layouts, editor automation).
- **Remaining / not yet verified by a human:** visual QA of the styling in the DAW; listening QA.
